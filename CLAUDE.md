...
- **Further-strictness open points (cross-repo, not yet done).** Items below are tracked across all four Bernard-Ladenthin Java repos and can be picked up incrementally:
  ...
  - **Mutation-testing threshold enforcement (PIT)** — only `streambuffer` currently enforces 100 % mutation coverage. The other three repos have PIT available but no threshold. Decide a project-appropriate per-repo target.
  ...
  - **JPMS `module-info.java` with `@NullMarked` at module level** — none of the four repos have JPMS today. A larger refactor; the per-package `@NullMarked` we already added covers the same nullness scope at lower cost.
